...
try:
    ...
    logger.warning(f"⚠️ Multi-format ingestion modules not available: {e}")
        ...
        try:
            ...
            if query:
                ...
                try:
                    ...
                    msg = str(inner_e)
                    # Handle common 'table not found' database errors and try a fallback
                    if 'does not exist' in msg or 'UndefinedTable' in msg or 'no such table' in msg:
                        logger.warning(f"   ⚠️ Table {schema + '.' if schema else ''}{table_name} not found: {inner_e}")
                        # Retry without schema (some environments expose tables in public/default schema)
                        try:
                            logger.info(f"   ℹ️ Retrying extraction without schema for table '{table_name}'")
                            df = sql_ingester.extract_table(table_name, schema=None, where_clause=where_clause)
                        except Exception as inner_e2:
                            # Try to list available tables across all schemas for diagnostics
                            try:
                                all_tables = sql_ingester.list_tables(schema=None)
                                available_msg = f"\n   Available tables (all schemas): {all_tables[:20]}"  # Limit to first 20
                                if len(all_tables) > 20:
                                    available_msg += f" ... and {len(all_tables) - 20} more"
                            except Exception as list_err:
                                logger.debug(f"   Could not list tables: {list_err}")
                                available_msg = ""
                            
                            error_msg = f"Table {schema + '.' if schema else ''}{table_name} not found.{available_msg}"
                            logger.error(f"   ❌ SQL extraction failed: {error_msg}")
                            raise RuntimeError(error_msg) from inner_e2
                    else:
                        raise
